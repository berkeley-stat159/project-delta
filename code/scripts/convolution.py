...
for subject, run in subjects, runs:
    ...
    # Create neural time courses for each of the three regressors of interest
    time_course_gain = obj.time_course(gain, step_size)
    time_course_loss = obj.time_course(gain, step_size)
    time_course_dist = obj.time_course(gain, step_size)
    ...
    # Create figures plotting the predicted HRF signal on the variable time:
    ...
    plt.xlabel("Time")
    plot.ylabel("Hemodynamic Response")
    ...
    path_results = "results/sub%s_run%s" % (subject, run)
    if not os.path.exists(path):
        os.makedirs(path)

    plt.savefig(path + "convolution.png", dpi=500)
    plt.close()

    # Save txt files to results directory
    np.savetxt(path_results + "conv_gain.txt", neural_gain)
    np.savetxt(path_results + "conv_loss.txt", neural_loss)
    np.savetxt(path_results + "conv_dist2indiff.txt", neural_dist2indiff)
